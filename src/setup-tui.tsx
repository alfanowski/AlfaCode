...
type Stage = "provider" | "base-url" | "api-key" | "connecting" | "error";
...
function ProviderSetup({ descriptors, notice, connect }: ProviderSetupOptions): React.JSX.Element {
  ...
  useInput((input, key) => {
    ...
    if (stage === "error") {
      if (key.return) { setError(undefined); setValue(""); setStage("api-key"); }
      return;
    }
    if (stage === "provider") {
      ...
      if (key.return) {
        ...
        setSelected(descriptor); setBaseUrl(undefined); setValue(descriptor.suggestedBaseUrl ?? ""); setStage(descriptor.requiresBaseUrl ? "base-url" : "api-key");
        return;
      }
  ...
  return <Box flexDirection="column" paddingX={1}>
    ...
    {stage === "error" ? <><Text color={colors.danger}>Connection failed: {error}</Text><Text color={colors.muted}>Press enter to retry the API key.</Text></> : null}
  </Box>;
}
