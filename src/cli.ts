...
// Exported (rather than left as a private helper) only to satisfy `noUnusedLocals`: after this
// task's edits it has no remaining caller in this file. Task 2 removes it entirely alongside the
// zero-provider passthrough it used to gate; see the note on `classicLaunch` above.
export async function selectedCredentialUnavailable(
