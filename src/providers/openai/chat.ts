...
export class OpenAIChatAdapter implements Provider {
  ...
  public async countTokens(request: ProviderMessageRequest, _context: ProviderRequestContext): Promise<TokenCount> {
    // Generic Chat Completions exposes no portable count endpoint; keep the capability marked false.
    return { inputTokens: Math.ceil(JSON.stringify(toChatRequest(request)).length / 4), source: "estimated", exact: false };
  }
