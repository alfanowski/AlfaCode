...
function ChatTui({ session, identity, config, models, permissions, loadUsage, fullscreen = false, resolveAction }: ChatTuiOptions & { readonly resolveAction: (action: ChatAction) => void }): React.JSX.Element {
  ...
  useInput((text, key) => {
    ...
    // exactly how that was confirmed against Ink's source). Handling it here, first, is the only
    // way to keep a drag's coordinate bytes from otherwise falling through to the composer-insertion
    // branch far below and being typed in as literal garbage — every other useInput listener would
    // still fire independently since Ink fans this event out to all of them.
  ...
  function handleMouseCopyCommand(command: string): void {
    const [, sub] = command.split(/\s+/u).filter((token) => token.length > 0);
    if (sub !== undefined && sub !== "on" && sub !== "off") { appendSystem(setMessages, "Usage: /mousecopy [on|off]"); return; }
    ...
    appendSystem(setMessages, `Copy on mouse-select: ${enabled ? "on" : "off"}`);
  }
  /**
  ...
  function handleMouseSelectionComplete(span: MouseSelectionSpan): void {
    ...
    void writeSystemClipboardText(text)
      .then((result) => {
        ...
        appendSystem(setMessages, result.truncated ? `${label} (truncated to fit terminal limits)` : label);
      })
      .catch((error: unknown) => {
        appendSystem(setMessages, `Mouse-select copy failed: ${error instanceof Error ? error.message : String(error)}`);
      });
  }
