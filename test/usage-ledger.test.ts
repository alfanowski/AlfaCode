...
describe("UsageLedger", () => {
  it("replaces cumulative snapshots and aggregates final provider usage", async () => {
    ...
    const attempt = await store.start({ session: "session-private", agent: "worker-a", providerId: "google", routeModelId: "alfacode-anthropic/google/gemini-test", upstreamModel: model.id, model, requestedOutputTokens: 100 });
    ...
    expect(result.attempts[0]).toMatchObject({ outcome: "completed", usageCompleteness: "final", responseStarted: true, requestedOutputTokens: 100 });
    await store.close();
  });

  it("keeps missing usage and cancellation distinguishable from a zero-token result", async () => {
    const { ledger: store } = await ledger();
    const attempt = await store.start({ session: "session-private", agent: "main", providerId: "google", routeModelId: "route", upstreamModel: model.id, model });
  ...
  it("persists only pseudonymous metadata and recovers a corrupt database", async () => {
    ...
    const attempt = await store.start({ session: secret, agent: "agent-private", providerId: "google", routeModelId: "route", upstreamModel: model.id, model });
